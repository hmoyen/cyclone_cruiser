...
sonar_baseline = 0.2  # Distance between sonar sensors (meters)
...
class RotationListener(Node):
    ...
    def listener_callback(self, msg):
        ...
        # Update position and orientation using wheel odometry
        ...
        # Correct position and orientation with sonar data
        self.update_with_sonar()

        # Publish the updated position as part of the path
        pose = PoseStamped()
        ...
        # Optionally, add current position to path
        self.path_msg.poses.append(pose)
        self.path_msg.header.stamp = self.get_clock().now().to_msg()

        # Publish path to /path topic
    ...
    def sonar1_callback(self, msg):
        self.add_sonar_point(msg, angle_offset=90.0)

    def sonar2_callback(self, msg):
        self.add_sonar_point(msg, angle_offset=-90.0)

    def add_sonar_point(self, msg, angle_offset):
    ...
    def update_with_sonar(self):
        if len(self.sonar_points) < 2:
            return  # Need both sonar data to update

        # Extract the latest sonar points
        (sonar1_x, sonar1_y) = self.sonar_points[-2]
        (sonar2_x, sonar2_y) = self.sonar_points[-1]

        # Calculate distances to walls (assume y-axis walls)
        d_L = abs(sonar1_y - self.y)
        d_R = abs(sonar2_y - self.y)

        # Estimate orientation correction (assuming parallel walls)
        delta_theta = math.atan2((d_R - d_L), sonar_baseline)
        self.theta += math.degrees(delta_theta)
        self.theta = (self.theta + 180) % 360 - 180  # Normalize to [-180, 180]

        # Optionally adjust position based on sonar data
        wall_center = (d_L + d_R) / 2
        self.y = wall_center
